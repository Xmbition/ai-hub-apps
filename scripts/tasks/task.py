...
from .util import SHELL_EXECUTABLE, debug_mode, echo, have_root
...
class RunCommandsWithVenvTask(RunCommandsTask):
    ...
    def __init__(
        ...
        if self.venv is not None:
            if platform == "win32":
                self.commands = [
                    f". {self.venv}\\Scripts\\Activate.ps1; {command}"  # && is failing in pwsh
                    for command in self.commands
                ]
            else:
                self.commands = [
...
class PyTestTask(RunCommandsWithVenvTask):
    ...
    def __init__(
        ...
        default_options = "-rvv -p no:warnings --durations-min=0.5 --durations=20"
        if debug_mode():
