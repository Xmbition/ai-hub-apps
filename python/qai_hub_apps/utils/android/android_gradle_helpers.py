...
from qai_hub_apps.configs.info_yaml import QAIHAAppInfo
from qai_hub_apps.configs.versions_yaml import VersionsRegistry
from qai_hub_apps.utils.models.install_model import install_model
...
def extract_api_versions(app_root: str | os.PathLike) -> dict[str, str]:
    ...
    if not result or process.returncode != 0:
        raise ValueError(
            f"Unable to extract API versions for {app_root}. Make sure \"apply from: '../common.gradle'\" is included in build.gradle. Errors:\n{process.stderr}"
        )
